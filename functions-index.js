...
/**
 * 会話作成時にカルテのallowedFortuneTellersを更新
 * Firestore Trigger: conversations/{conversationId} onCreate
 */
exports.addFortuneTellerToKarte = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    try {
      ...
      if (!clientId || !fortuneTellerId) {
        console.log('clientIdまたはfortuneTellerIdが見つかりません');
        return null;
      }

      const db = admin.firestore();
      const karteRef = db.collection('kartes').doc(clientId);
      
      // カルテが存在するか確認
      const karteSnap = await karteRef.get();
      
      if (karteSnap.exists()) {
        // 既存のallowedFortuneTellersを取得（なければ空配列）
        const currentAllowed = karteSnap.data().allowedFortuneTellers || [];
        
        // 占い師がまだリストに含まれていなければ追加
        if (!currentAllowed.includes(fortuneTellerId)) {
          await karteRef.update({
            allowedFortuneTellers: admin.firestore.FieldValue.arrayUnion(fortuneTellerId)
          });
          console.log(`カルテに占い師を追加: ${fortuneTellerId} → ${clientId}`);
        }
      } else {
        // カルテが存在しない場合は作成
        await karteRef.set({
          clientId: clientId,
          allowedFortuneTellers: [fortuneTellerId],
          createdAt: admin.firestore.FieldValue.serverTimestamp()
        });
        console.log(`新しいカルテを作成: ${clientId}`);
      }
...
/**
 * 既存の会話データからkarteのallowedFortuneTellersを作成（一回だけ実行）
 * HTTP関数として実装
 * 
 * 使い方:
 * https://YOUR-PROJECT.cloudfunctions.net/migrateExistingConversationsToKartes?adminKey=YOUR_ADMIN_KEY
 */
exports.migrateExistingConversationsToKartes = functions.https.onRequest(async (req, res) => {
  // CORS対応
  ...
  try {
    ...
    console.log('移行開始: 全ての会話を取得中...');
    
    // 全ての会話を取得
    const conversationsSnapshot = await db.collection('conversations').get();
    console.log(`${conversationsSnapshot.size}件の会話を取得しました`);
    ...
    for (const convDoc of conversationsSnapshot.docs) {
      ...
      if (!clientId || !fortuneTellerId) {
        console.log(`スキップ: clientIdまたはfortuneTellerIdがありません - ${convDoc.id}`);
        continue;
      }
      
      // クライアントごとにallowedFortuneTellersを集める
      if (!updates[clientId]) {
        updates[clientId] = new Set();
      }
      updates[clientId].add(fortuneTellerId);
    }
    
    console.log(`${Object.keys(updates).length}件のカルテを更新します...`);
    
    // 各カルテを更新
    for (const [clientId, fortuneTellerIds] of Object.entries(updates)) {
      const karteRef = db.collection('kartes').doc(clientId);
      const karteDoc = await karteRef.get();
      
      if (karteDoc.exists()) {
        // 既存のallowedFortuneTellersとマージ
        const existingAllowed = karteDoc.data().allowedFortuneTellers || [];
        const allAllowed = [...new Set([...existingAllowed, ...Array.from(fortuneTellerIds)])];
        
        await karteRef.update({
          allowedFortuneTellers: allAllowed
        });
        console.log(`更新: ${clientId} - ${allAllowed.length}人の占い師`);
      } else {
        // カルテが存在しない場合は作成
        await karteRef.set({
          clientId: clientId,
          allowedFortuneTellers: Array.from(fortuneTellerIds),
          createdAt: admin.firestore.FieldValue.serverTimestamp()
        });
        console.log(`作成: ${clientId} - ${fortuneTellerIds.size}人の占い師`);
      }
    ...
    res.json({
      success: true,
      message: `${processedCount}件のカルテを更新しました`,
